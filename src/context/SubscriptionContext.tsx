...
export const SubscriptionProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchSubscriptionStatus = async (tokenOverride?: string) => {
    try {
      // Prioridade: tokenOverride > authToken do contexto > AsyncStorage
      let token = tokenOverride || authToken;

      if (!token) {
        // Fallback para AsyncStorage se não tiver token no contexto
        token = await AsyncStorage.getItem("@FutScore:token");
      }

      if (!token) {
        console.log(
          "[SubscriptionContext] No token found, setting loading to false",
        );
        setLoading(false);
        return;
      }
      ...
      console.error(
        "[SubscriptionContext] Erro ao buscar status de assinatura:",
        error,
      );
    } finally {
      setLoading(false);
  ...
  const fetchMaintenanceStatus = async () => {
    try {
      console.log(
        "[SubscriptionContext] Checking channels maintenance status...",
      );
      // Note: A rota está montada em /admin, não em /api/admin
      const response = await axios.get(
        `${API_BASE_URL}/admin/system-settings/channels_maintenance`,
      );

      const isMaintenance = response.data?.value === true;
      console.log(
        "[SubscriptionContext] Channels maintenance response:",
        response.data,
      );
      ...
      console.error(
        "[SubscriptionContext] Error checking maintenance:",
        error?.message || error,
      );
  ...
  const fetchCheckoutUrl = async () => {
    try {
      ...
      if (!token) {
        console.log(
          "[SubscriptionContext] ERRO: Token não encontrado no AsyncStorage",
        );
        return null;
      }
      ...
      console.error(
        "[SubscriptionContext] ERRO ao buscar URL de checkout:",
        error?.message,
      );
      return null;
    }
  ...
  const activateTrial = async (): Promise<{
    success: boolean;
    message: string;
    trialEndDate?: string;
  }> => {
    ...
    try {
      ...
      console.log(
        "[SubscriptionContext] Trial ativado com sucesso:",
        response.data,
      );
      ...
      console.error(
        "[SubscriptionContext] Erro ao ativar trial:",
  ...
  useEffect(() => {
    if (!authLoading && user && authToken) {
      // Usa o token diretamente do contexto de Auth para evitar race condition
      console.log(
        "[SubscriptionContext] Auth ready with token, fetching subscription status...",
      );
      fetchSubscriptionStatus(authToken);
    } else if (!authLoading && !user) {
      // Se deslogou ou não tem user, reseta status
      console.log(
        "[SubscriptionContext] No user, resetting subscription status",
      );
      setSubscriptionStatus({ isPremium: false, hasSubscription: false });
      setLoading(false);
    }
  ...
  const claimGift = async (): Promise<{
    ...
    try {
      ...
      console.log(
        "[SubscriptionContext] Gift resgatado com sucesso:",
        response.data,
      );
      ...
      console.error(
        "[SubscriptionContext] Erro ao resgatar gift:",
        error?.response?.data || error?.message,
      );
      return {
        success: false,
