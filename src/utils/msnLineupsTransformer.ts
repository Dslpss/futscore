...
export function transformMsnLineupsToLineups(msnLineupsData: any): Lineup[] {
  if (!msnLineupsData || !msnLineupsData.lineups || !Array.isArray(msnLineupsData.lineups)) {
    return [];
  }
  ...
  return msnLineupsData.lineups.map((lineupData: any) => {
    ...
    return {
      team: {
        ...
        name: lineupData.team?.name?.localizedName || lineupData.team?.name?.rawName || 'Unknown',
