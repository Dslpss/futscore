...
const SoccerField = ({ homeLineup, awayLineup, homeTeam, awayTeam }: { homeLineup: any, awayLineup: any, homeTeam: any, awayTeam: any }) => {
  ...
  const organizeByLine = (players: any[]) => {
     // Regex for positions
     ...
     if (lines[1].length === 0 && lines[2].length === 0 && lines[3].length === 0) {
       ...
       const remaining = players.length;
       const defCount = Math.floor(remaining * 0.4);
       const midCount = Math.floor(remaining * 0.35);
       
       let cursor = 0;
       lines[1] = players.slice(cursor, cursor + defCount); cursor += defCount;
       lines[2] = players.slice(cursor, cursor + midCount); cursor += midCount;
       lines[3] = players.slice(cursor);
     }
     
     return lines;
  };

  const renderTeam = (players: any[], isHome: boolean) => {
    ...
    return lines.map((line, lineIdx) => (
      line.map((player, pIdx) => {
        ...
        return (
          <View key={player.id} style={[styles.fieldPlayer, { left: left as any, top: top as any }]}>
                ...
                <>
                  ...
                  match.lineups && match.lineups.length > 0 ? (
                    <View style={styles.lineupsContainer}>
                      ...
                      {lineupView === "field" ? (
                        <SoccerField 
                          homeLineup={match.lineups[0]} 
                          awayLineup={match.lineups[1]}
