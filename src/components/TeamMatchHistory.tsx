...
export const TeamMatchHistory: React.FC<TeamMatchHistoryProps> = ({ 
  ...
  const loadMatchHistory = async () => {
    ...
    try {
      ...
      // Try to infer MSN ID if not provided
      const effectiveMsnId = msnId || inferMsnTeamId(teamId, teamName);

      // Try MSN Sports API first if we have MSN Team ID
      if (effectiveMsnId) {
        console.log(`[TeamMatchHistory] Trying MSN API for team ${effectiveMsnId}...`);
        const { msnSportsApi } = await import('../services/msnSportsApi');
        const { transformMsnGameToMatch } = await import('../utils/msnTransformer');
        
        try {
          const msnGames = await msnSportsApi.getTeamLiveSchedule(effectiveMsnId, 10);
          
          // Separate finished and upcoming games
          const finishedGames = msnGames.filter((game: any) => 
            game.gameState?.gameStatus === 'Final'
          );
          
          const upcomingGames = msnGames.filter((game: any) =>
            game.gameState?.gameStatus === 'PreGame'
          );
          ...
          console.log(`[TeamMatchHistory] MSN API returned ${finishedMatches.length} finished + ${upcomingMatches.length} upcoming games`);
        } catch (msnError) {
          console.log(`[TeamMatchHistory] MSN API failed, falling back to football-data.org:`, msnError);
        }
      }

      // Fallback to football-data.org if MSN didn't work or no MSN ID
      if (data.length === 0) {
        console.log(`[TeamMatchHistory] Trying football-data.org for team ${teamId}...`);
      ...
      console.log(`[TeamMatchHistory] Could not load matches for team ${teamId}:`, error);
      setMatches([]);
    } finally {
      setLoading(false);
    }
        ...
        {matches.map((match, index) => {
          ...
          return (
            <View key={match.fixture.id} style={styles.matchCard}>
              <LinearGradient
                colors={['rgba(255,255,255,0.05)', 'rgba(255,255,255,0.02)'] as any}
